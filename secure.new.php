...
$PageTitle = 'Login - AnimeFTW.TV';
	if (isset($_POST['username']) && isset($_POST['password'])){
		...
		if ( array_key_exists ( '_submit_check', $_POST ) )
		{
			...
			{
				//change_password, new_password, confirm_new_password, password
				if(isset($_POST['change_password']) && $_POST['change_password'] == 1)
				{
					// first thing is to check to make sure everything was submitted correctly.
					if(!isset($_POST['new_password']) || !isset($_POST['confirm_new_password']) || !isset($_POST['password']))
					{
						// something wasn't set.. let them know that something was wrong.
						$error = 'A Field was not filled out, please fill everything out and try again.';
						$failed = FALSE;
						$PWDChange = TRUE;
					}
					else
					{
						// Check the new password, make sure it matches.
						if($_POST['new_password'] != $_POST['confirm_new_password'])
						{
							// passwords did not match, let them know
							$error = 'The Password confirmation did not match your new password, please make sure they match and try again.';
							$failed = FALSE;
							$PWDChange = TRUE;
						}
						else
						{
							// we need to check the account, now that the previous creds are a success, make sure the account can authenticate as it is.
							if(isEmail($userName))
							{
							...
							if ( $db->RecordCount ( $query ) == 1 )
							{
								...
								if ( $row->Active == 1 )
								{
									// the password was proper, we now need to reset the password and then push them through.
				...
				{
					// normal login, let's try it out.
					if(isEmail($userName))
					{
						// if the username is an email, we change the query string a bit.
						$query = 'SELECT `ID`, `Username`, `Active`, `Reason`, `Password`, `ForcePasswordChange` FROM ' . DBPREFIX . 'users WHERE Email = \'' . mysql_real_escape_string( $userName ) . '\' AND Password = \'' . mysql_real_escape_string( md5 ( $password ) ) . '\'';
					}
					else
					{
						$userName = makeUrlFriendly($userName);
						// if the username is a real username.. then we use a different string.
						$query = 'SELECT `ID`, `Username`, `Active`, `Reason`, `Password`, `ForcePasswordChange` FROM ' . DBPREFIX . 'users WHERE Username = \'' . mysql_real_escape_string( $userName ) . '\' AND Password = \'' . mysql_real_escape_string( md5 ( $password ) ) . '\'';
					}
					...
					if ( $db->RecordCount ( $query ) == 1 )
					{
						...
						if ( $row->Active == 1 )
						{
							...
							{
								// The user is set to force a password change, do not allow the login to proceed.
								$error = 'Your account requires a password change before you can log in again. <br /> Please fill out the form and submit the password change request to submit your new password.<br />You will be automatically logged in on a successful submission.';
								$failed = FALSE;
								$PWDChange = TRUE;
							}
							else
	...
	if(isset($_GET['subnode']) && $_GET['subnode'] == 'submit'){
		...
		if(md5($uid) == $ver){		
			...
			header('location: http://'.$_SERVER['HTTP_HOST'].'/reviews');
		}
	}
}
	...
	$query = "SELECT * FROM users WHERE ID = " . $db->qstr ( $_GET['ID'] );
	...
	if($_GET['node'] == 'login')
	{
		...
		else 
		{
			...
			if($PWDChange == TRUE)
			{
				// the user is to change their password to login, let's give it to them.
				echo '
				<input name="change_password" id="change_password" type="hidden" value="1" />
				<tr>
					<td align="right">
					<label class="left" for="new_password" style="margin: 0px 0px 0px 0px;color:#555555;">New Password:</label>
					</td>
					<td width="340px" align="left">
					<input name="new_password" id="new_password" type="text" class="loginForm" style="width: 227px" /><br />
					</td>
				</tr>
				<tr>
					<td align="right">
					<label class="left" for="confirm_new_password" style="margin: 0px 0px 0px 0px;color:#555555;">Confirm New Password:</label>
					</td>
					<td width="340px" align="left">
					<input name="confirm_new_password" id="confirm_new_password" type="text" class="loginForm" style="width: 227px" /><br />
					</td>
				</tr>
				<tr>
					<td align="right">
					<input name="last_page" type="hidden" value="'.$_POST['last_page'].'" />
					<input name="username" id="username" type="hidden" value="' . $_POST['username'] . '" />
					...
					<td align="left">
						...
						<input name="submit" type="submit" class="button_2" value="Change Password and Log in" />
					</td>
				</tr>
				<tr>
					<td colspan="2">
			...
			{
				echo '
				<input name="change_password" id="change_password" type="hidden" value="0" />
				<tr>
					<td align="right">
    ...
    echo '<div align="center"><form id="form1" action="'.$_SERVER['REQUEST_URI'].'" method="post">
		...
		echo '<div align="center">As the site has grown, so has our offering, many users asked us, how could we support the site, one the devoted itself to the highest quality anime on the net. After much thought, we came up with the Advanced Member Title. Since the conception of the Advanced Member, we have evolved the status to something more. FTW Subscribers, users who are a FTW Subscriber have access to all of the premium benefits on all of FTW Entertainment LLC\'s Sites, Being an Advanced Member is a Subsidary of this overall perk.</div> <br /><br />
				...
				<div align="left"><a href="/images/externalamdownload.jpg" target="_blank"><img src="/images/externalamdownload.jpg" width="350" height="83" alt="External AM downloads" style="float:left;" /></a><br /><b>Know What episodes you love?</b> <br />Download them from the episodes listing in a flash!</div><br /><br />
				<br />
				<div align="right"><a href="/images/internalamdownload.jpg" target="_blank"><img src="/images/internalamdownload.jpg" width="350" height="27" alt="Internal AM downloads" style="float:right;" /></a><b>Watched an Episode you know you will watch again and again?</b> <br />Download links are provided at the top of each and every video!!</div><br /><br />
				<div align="left"><a href="/images/dropdownamoptions.jpg" target="_blank"><img src="/images/dropdownamoptions.jpg" width="350" height="230" alt="AM Dropdown" style="float:left;" /></a><br /><b>Love profiles?</b> <br />With even more options for your account, go nuts!</div><br /><br />
				<br /><br />
...
<br />
<div>
Since becoming an Advanced Member is a product of being a FTW Subscriber, we are asking everyone to continue over to <a href="http://members.ftwentertainment.com">FTW Entertainment\'s Subscriber Zone</a> To continue with the signup process, once your account has been updated as a FTW Subscriber you will see the AnimeFTW.TV Advanced Status come to life.</div><br /><div align="center"><h2><a href="http://members.ftwentertainment.com">Continue to FTW Entertainment\'s Subscriber Zone</a></h2></div>';
		echo '</div>';
}
else if($_GET['node'] == 'reviews'){
