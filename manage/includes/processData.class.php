...
class processData extends Config {
	...
	private function processPostedData()
	{
		...
		else if($_POST['method'] == 'EditSeries' && $this->ValidatePermission(23) == TRUE)
		{
			if(isset($_POST['Authorization']) && $_POST['Authorization'] == '0110110101101111011100110110100001101001')
			{
				...
				$ueid = mysqli_real_escape_string($conn, $_POST['uploadsEntry']);
		...
		else if($_POST['method'] == 'AddSeries' && $this->ValidatePermission(22) == TRUE)
		{
			if(isset($_POST['Authorization']) && $_POST['Authorization'] == '0110110101101111011100110110100001101001')
			{
				...
				mysqli_query($conn, $query) or die('There was an error adding the series: ' . mysqli_error() . ', here was the query: ' . $query);
		...
		else if($_POST['method'] == 'MassEpisodeUpdate' && $this->ValidatePermission(72) == TRUE)
		{
			if(isset($_POST['Authorization']) && $_POST['Authorization'] == '0110110101101111011100110110100001101001')
			{
				...
				$old_hd = mysqli_real_escape_string($conn, $_POST['old_hd']);
		...
		else if($_POST['method'] == 'UploadsAddition')
		{
			if(isset($_POST['Authorization']) && $_POST['Authorization'] == '0110110101101111011100110110100001101001')
			{
				...
				$query = "INSERT INTO uestatus (`series`, `prefix`, `episodes`, `type`, `resolution`, `status`, `user`, `updated`, `anidbsid`, `fansub`, `sid`, `change`, `hd`, `airing`) VALUES ('" . mysqli_real_escape_string($conn, $_POST['Series']) . "', '" . mysqli_real_escape_string($conn, $_POST['Prefix']) . "', '" . $episodes . "', '" . mysqli_real_escape_string($conn, $_POST['Type']) . "', '" . $dimmensions . "', '" . mysqli_real_escape_string($conn, $_POST['Status']) . "', '" . mysqli_real_escape_string($conn, $_POST['user']) . "', NOW(), '" . mysqli_real_escape_string($conn, $_POST['anidb']) . "', '" . mysqli_real_escape_string($conn, $_POST['fansub']) . "', '" . mysqli_real_escape_string($conn, $_POST['sid']) . "', " . $Change . ", '" . mysqli_real_escape_string($conn, $_POST['hdresolution']) . "', '" . mysqli_real_escape_string($conn, $_POST['seriesstatus']) . "')";
				mysqli_query($conn, $query) or die(mysqli_error());

				$this->ModRecord('Added a Series to the Tracker, ' . mysqli_real_escape_string($conn, $_POST['Series']));
		...
		else if($_POST['method'] == 'UploadsEdit')
		{
			if(isset($_POST['Authorization']) && $_POST['Authorization'] == '0110110101101111011100110110100001101001')
			{
				...
				`sid` = '" . mysqli_real_escape_string($conn, $_POST['sid']) . "',
				`change` = " . $Change. ",
				`hd` = '" . mysqli_real_escape_string($conn, $_POST['hdresolution']) . "',
				`airing` = '" . mysqli_real_escape_string($conn, $_POST['seriesstatus']) . "'
				...
				$query = "UPDATE `requests` SET `status` = '" . array_search($_POST['Status'], $EntryStatuses) . "' WHERE `anidb` = " . mysqli_real_escape_string($conn, $_POST['anidb']);
				mysqli_query($conn, $query) or die(mysqli_error());

				$this->ModRecord('Editted Tracker entry ' . mysqli_real_escape_string($conn, $_POST['ueid']));
		...
		else if($_POST['method'] == 'UserEdit')
		{
			...
			{
				$query = "SELECT Password, Level_access FROM users WHERE ID = " . mysqli_real_escape_string($conn, $_POST['s']);
				$result = mysqli_query($conn, $query);
				...
				if((isset($_POST['Authorization']) && $_POST['Authorization'] == md5($_SERVER['REMOTE_ADDR'].$row['Password'].$_SERVER['HTTP_USER_AGENT']) && ($row['Level_access'] != 1 && $row['Level_access'] != 2)) || ((isset($_POST['Authorization']) && $_POST['Authorization'] == '0110110101101111011100110110100001101001') && ($row['Level_access'] == 1 || $row['Level_access'] == 2)))
				{
					...
					if($row['Level_access'] == 1 || $row['Level_access'] == 2){
						$additional = ', avatarActivate=\'' . mysqli_real_escape_string($conn, $avataractive) . '\', avatarExtension=\'' . mysqli_real_escape_string($conn, $avatarextension) . '\', personalMsg=\'' . mysqli_real_escape_string($conn, $personalmsg) . '\', memberTitle=\'' . mysqli_real_escape_string($conn, $membertitle) . '\', aboutMe=\'' . mysqli_real_escape_string($conn, $aboutme) . '\', interests=\'' . mysqli_real_escape_string($conn, $interests) . '\', signatureActive=\'' . mysqli_real_escape_string($conn, $sigactive) . '\', Signature=\'' . mysqli_real_escape_string($conn, $Signature) . '\'';
					}
					else if($row['Level_access'] != 3 && $row['Level_access'] != 1 && $row['Level_access'] != 2){
					//AMs and staff can see these...
						if($Signature != '')
						{
							$additional = ', aboutMe=\'' . mysqli_real_escape_string($conn, $aboutme) . '\', interests=\'' . mysqli_real_escape_string($conn, $interests) . '\', Signature=\'' . mysqli_real_escape_string($conn, $Signature).'\', signatureActive=\'yes\'';
						}
						else
						{
							$additional = ', aboutMe=\'' . mysqli_real_escape_string($conn, $aboutme) . '\', interests=\'' . mysqli_real_escape_string($conn, $interests) . '\', Signature=\'' . mysqli_real_escape_string($conn, $Signature).'\'';
						}
					}
					else {
					...
					showChat=\'' . mysqli_real_escape_string($conn, $showChat) . '\',
	...
	private function addEpisode(){
		...
		$fromep = $_POST['fromep'];
		$toep = $_POST['toep'];
		$epnumber = mysqli_real_escape_string($conn, $_POST['epnumber']);
		$sid = mysqli_real_escape_string($conn, $_POST['sid']);
		$epname = mysqli_real_escape_string($conn, urldecode($_POST['epname']));
		$vidheight = mysqli_real_escape_string($conn, $_POST['vidheight']);
		$vidwidth = mysqli_real_escape_string($conn, $_POST['vidwidth']);
		$epprefix = mysqli_real_escape_string($conn, $_POST['epprefix']);
		$subGroup = mysqli_real_escape_string($conn, $_POST['subGroup']);
		...
		$addtime = mysqli_real_escape_string($conn, $_POST['date']);
		$videotype =mysqli_real_escape_string($conn,  $_POST['videotype']);
		$hd = mysqli_real_escape_string($conn, $_POST['hd']);
		if($auto){
		...
		for($i=$fromep;$i<=$toep;$i++){
			$epnumber = mysqli_real_escape_string($conn, $i);
			...
			$airingCheck = mysqli_query($conn, "SELECT episode.id AS epid, series.id AS sid, series.stillRelease, series.fullSeriesName FROM series, episode WHERE series.id = '".$sid."' AND episode.sid = series.id AND episode.epnumber = '".$epnumber."'");
			...
			if($ar['stillRelease'] == 'yes')
			{
				...
				mysqli_query($conn, "INSERT INTO email (`id`, `date`, `sid`, `v1`, `v2`) VALUES (NULL,'" . time() . "', '" . $ar['sid'] . "', '" . $ar['epid'] . "', '1');");
			}
			...
			if($Changed == 'on')
			{
                ...
                $query = "UPDATE uestatus SET `change` = 0 WHERE ID = " . mysqli_real_escape_string($conn, $_POST['ueid']);
				mysqli_query($conn, $query);
