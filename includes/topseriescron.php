<?php
	...
	if($onlyonce == 1)
	{
		...
		while($correctorder <= 5)
		{
			if($correctorder == 1)
			{
				...
				$allzeros = mysqli_query($conn, "UPDATE topseriescalc SET countedPages='0', pagePercentage='0'");
				echo "done clearing that... begin deleting of old not needed episode logs.<br /> \n";
				$correctorder++;
			}
			elseif($correctorder == 2)
			{
				$prefix = mysqli_query($conn, "DELETE FROM episodestats WHERE date<'".$currentstrtotime."'");
	...
	#$killtable = mysqli_query($conn, "TRUNCATE TABLE episodestats");
	mysqli_query($conn, "INSERT INTO crons_log (`id`, `cron_id`, `start_time`, `end_time`) VALUES (NULL, '12', '" . $starttime . "', '" . time() . "');");
	mysqli_query($conn, "UPDATE crons SET last_run = '" . time() . "', status = 0 WHERE id = 12");
include 'closedb.php';
?>