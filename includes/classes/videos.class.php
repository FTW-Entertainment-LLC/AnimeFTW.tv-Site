...
class AFTWVideos extends Config{
    ...
    var $watchListStatuses = array();
    
    public function __construct()
    {
        parent::__construct();
        $this->buildCategories();
        $this->watchListStatuses();
    }
    ...
    public function showListing ($listType,$sort,$alevel,$stype,$url = NULL){
        if($alevel == 0){$aonly = "AND `series`.`aonly` = '0'";}
        else if ($alevel == 3){$aonly = "AND `series`.`aonly` <= '1'";}
        else{$aonly = '';}
        ...
        else if($stype == 1)
        {
            ...
            if($sort == 'airing'){
                $sql = "SELECT UPPER(SUBSTRING(fullSeriesName,1,1)) AS letter, id, fullSeriesName, seoname, description, stillRelease, seriesType, seriesList, moviesOnly FROM series WHERE seriesList='$listType' AND active='yes' AND stillRelease = 'yes' ".$aonly."ORDER BY fullSeriesName";
            }
            else if($sort == 'completed'){
                $sql = "SELECT UPPER(SUBSTRING(fullSeriesName,1,1)) AS letter, id, fullSeriesName, seoname, description, stillRelease, seriesType, seriesList, moviesOnly FROM series WHERE seriesList='$listType' AND active='yes' AND stillRelease = 'no' ".$aonly."ORDER BY fullSeriesName";
            }
            else if($sort == 'movies'){
                $sql = "SELECT UPPER(SUBSTRING(fullSeriesName,1,1)) AS letter, id, fullSeriesName, seoname, description, stillRelease, seriesType, seriesList, moviesOnly FROM series WHERE seriesList='$listType' AND active='yes' AND Movies > 0 ".$aonly."ORDER BY fullSeriesName";
            }
            else {
                $sql = "SELECT UPPER(SUBSTRING(fullSeriesName,1,1)) AS letter, id, fullSeriesName, seoname, description, stillRelease, seriesType, seriesList, moviesOnly FROM series WHERE seriesList='$listType' AND active='yes' ".$aonly."AND category LIKE '% ".$catsort." %' ORDER BY seriesName";
            }
        }
        else {
            if($sort == NULL){
                $sql = "SELECT UPPER(SUBSTRING(fullSeriesName,1,1)) AS letter, id, fullSeriesName, seoname, description, stillRelease, seriesType, seriesList, moviesOnly FROM series WHERE seriesList='$listType' AND active='yes' ".$aonly."ORDER BY fullSeriesName";
            }
            else {
                $sql = "SELECT UPPER(SUBSTRING(fullSeriesName,1,1)) AS letter, id, fullSeriesName, seoname, description, stillRelease, seriesType, seriesList, moviesOnly FROM series WHERE seriesList='$listType' AND active='yes' ".$aonly."AND ratingLink LIKE '%".mysql_real_escape_string($sort)."%' ORDER BY seriesName";
            }
        }
        ...
        while ($records = @mysql_fetch_assoc($query)) {
            ...
            // Structure the series to make them easier to parse.
            $seriesArray[$records['letter']][$records['fullSeriesName']]['id'] = $records['id'];
            $seriesArray[$records['letter']][$records['fullSeriesName']]['fullSeriesName'] = $records['fullSeriesName'];
            $seriesArray[$records['letter']][$records['fullSeriesName']]['seoname'] = $records['seoname'];
            $seriesArray[$records['letter']][$records['fullSeriesName']]['description'] = $records['description'];
            $seriesArray[$records['letter']][$records['fullSeriesName']]['stillRelease'] = $records['stillRelease'];
        ...
        foreach(range('0','9') as $i) {
            echo (array_key_exists ("$i", $alpha)) ? '<a href="#'.$i.'" title="'.$alpha["$i"].' results" class="anime-sort-listing">'.$i.'</a> | ' : "$i | ";
        }
        echo (array_key_exists (".", $alpha)) ? '<a href="#." title="' . $alpha["."].' results" class="anime-sort-listing">.</a>' : ".";
        echo ' | ';
        ...
        $displayType = 1;
        echo '<div id="anime-listing-wrapper">';
        if($displayType == 0) {
            ...
            foreach(range('A','B') as $i) {
                echo $this->displayAnimeListingPosters($i,$seriesArray);
            }
            echo '
            ...
            <script type="text/javascript" src="/scripts/jquery.unveil.js?v=1.0.0"></script>
            <script>
            $(document).ready(function() {
                $("img").unveil(300);
            });
            ...
            <script>
            $(document).ready(function() {
                $(".anime-sort-listing").click(function() {
                    // load in the clicked upon anime
                    var href_link = $(this).attr("href").substring(1);
                    $("#anime-listing-wrapper").fadeOut("fast");
                    var dataObject = "letter=" + href_link;
                    $.ajax({
                        type: "POST",
                        url: "/scripts.php?view=dynamic-load&show=view-details&page=0&id=0",
                        data: dataObject,
                        cache: false,
                        success: function(html){
                            $("#anime-listing-wrapper").html(html);
                            $("#anime-listing-wrapper").fadeIn("slow");
                            return false;
                        }
                    });
                    return false;
    ...
    /*
    function displayAnimeListingPosters
    description: Gives back a list of anime in poster format.
    */
    private function displayAnimeListingPosters($i,$seriesArray) {
        $this->watchListStatuses[NULL] = 'untracked';
        echo '<div class="">';
    ...
    public function displaySpecificAnimeListingPosters($alpha) {
        ...
        while ($records = @mysql_fetch_assoc($query)) {
            ...
            // Structure the series to make them easier to parse.
            $seriesArray[$alpha][$records['fullSeriesName']]['id'] = $records['id'];
            $seriesArray[$alpha][$records['fullSeriesName']]['fullSeriesName'] = $records['fullSeriesName'];
            $seriesArray[$alpha][$records['fullSeriesName']]['seoname'] = $records['seoname'];
            $seriesArray[$alpha][$records['fullSeriesName']]['description'] = $records['description'];
            $seriesArray[$alpha][$records['fullSeriesName']]['stillRelease'] = $records['stillRelease'];
        ...
        echo $this->displayAnimeListingPosters($alpha,$seriesArray);
        echo '
            <script type="text/javascript" src="/scripts/jquery.unveil.js?v=1.0.0"></script>
            <script>
            $("img").unveil();
    ...
    private function showDownloadOption($epid,$CanDownload,$disk = FALSE)
    {
        if($CanDownload == TRUE)
        {
            ...
            if($disk == TRUE) {
                ...
                $data .= '<a href="#" onClick="window.location.href=\'//videos.animeftw.tv/download.php?series=' . $row['seriesName'] . '&preffix=' . $row['epprefix'] . '&epnumber=' . $row['epnumber'] . '&hd=480\'; return false;"><img src="' . $this->Host . '/' . $DLIcon . '" alt="Advanced Download" title="Click To download ' . $row['fullSeriesName'] . ' Episode ' . $row['epnumber'] . '" style="" border="0" /></a>';
            }
            return $data;
        }
    }
    
    private function watchListStatuses() {
        $query = "SELECT * FROM `watchlist_statuses`";
        $result = mysql_query($query);
        
        while(list($id, $StatusName) = mysql_fetch_array($result)) {
            $this->watchListStatuses[$id] = $StatusName;
        }
    }
}