...
class Episode extends Config {
	...
	public function array_displayEpisodes()
	{
		if(isset($this->Data['id']) && is_numeric($this->Data['id']))
		{
			...
			$this->mysqli->query("SET NAMES 'utf8'");
			$query = "SELECT `episode`.`id`, `episode`.`sid`, `episode`.`spriteId`, `episode`.`epname`, `episode`.`epnumber`, `episode`.`vidheight`, `episode`.`vidwidth`, `episode`.`epprefix`, `episode`.`subGroup`, `episode`.`Movie`, `episode`.`videotype`, `episode`.`image`, `episode`.`hd`, `episode`.`views`, `series`.`seriesname`, `sprites`.`width` as spriteWidth, `sprites`.`height` as spriteHeight, `sprites`.`totalWidth` as spriteTotalWidth, `sprites`.`rate` as spriteRate, `sprites`.`count` as spriteCount FROM `" . $this->MainDB . "`.`episode`, `" . $this->MainDB . "`.`series` LEFT JOIN `{$this->MainDB}`.`sprites` ON `sprites`.`id` = `episodes`.`spriteId` WHERE `episode`.`sid` = " . $this->mysqli->real_escape_string($this->Data['id']) . " AND `series`.`id`=`episode`.`sid` ORDER BY `episode`.`epnumber` LIMIT $startpoint, $count";
			//execute the query
			$result = $this->mysqli->query($query);
