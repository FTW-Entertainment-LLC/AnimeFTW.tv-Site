...
$subresults = $this->mysqli->query("SELECT (SELECT item_size FROM store_inventory WHERE id=store_orders_items.inventory_id) AS item_size, store_orders_items.quantity, store_items.name, store_items.price FROM store_orders_items, store_items WHERE store_orders_items.cart_id=" . $Var1 . " AND store_items.id=store_orders_items.item_id");		

while(list($item_size,$quantity,$name,$price,) = $subresults->fetch_assoc())
{
...
$this->mysqli->query("INSERT INTO email_logs (`id`, `date`, `script`, `action`) VALUES (NULL,'".time()."', '".$_SERVER['REQUEST_URI']."', 'AnimeFTW.tv Store Update for Cart " . $row['id'] . "');");
		}
		else if($Type == 2)
		{
			...
			$this->mysqli->query("INSERT INTO email_logs (`id`, `date`, `script`, `action`) VALUES (NULL,'".time()."', '".$_SERVER['REQUEST_URI']."', 'Video Image Creation Error Email');");
		} 
		else if($Type == 3)
		{
			...
			$this->mysqli->query("INSERT INTO email_logs (`id`, `date`, `script`, `action`) VALUES (NULL,'".time()."', '".$_SERVER['REQUEST_URI']."', 'Advanced Member Check');");
		} 
		else if($Type == 5)
		{
			...
			$this->mysqli->query("INSERT INTO email_logs (`id`, `date`, `script`, `action`) VALUES (NULL,'".time()."', '".$_SERVER['REQUEST_URI']."', 'Failed Login Attempt AFTW Management');");
		}
		else if($Type == 6)
		{
			// emails for the failing to log in to the Management console.
			...
			$this->mysqli->query("INSERT INTO email_logs (`id`, `date`, `script`, `action`) VALUES (NULL,'".time()."', '".$_SERVER['REQUEST_URI']."', 'AnimeFTW.tv Management Password Change Request');");
		}
		else if($Type == 7)
		{
			...
			$this->mysqli->query("INSERT INTO email_logs (`id`, `date`, `script`, `action`) VALUES (NULL,'".time()."', '".$_SERVER['REQUEST_URI']."', '" . $this->Subject . "');");
		}
		else
		{
