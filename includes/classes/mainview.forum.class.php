...
class AFTWForumMain extends Config {
	...
	public function MainDisplay(){
		...
		while(list($cid,$ctitle,$cpermission,$cseo) = mysql_fetch_array($result1)) {
			...
			$result200 = mysql_query($query200) or die('Error : ' . mysql_error());
			while(list($fid,$fpermission,$ftitle,$fdescription,$ficon,$fcid,$fseo) = mysql_fetch_array($result200)) {
				$fdescription = stripslashes($fdescription);
				$fim = "<img src='/images/forumimages/bf_new.png' border='0'  alt='Posts!' />";
				echo "<tr>\n";
				...
				$total_forum_threads = mysql_query("SELECT COUNT(pid) FROM forums_post WHERE pfid ='$fid' AND pistopic='1'"); 
				$total_forum_threads  = mysql_result($total_forum_threads , 0);
				$total_forum_posts = mysql_query("SELECT COUNT(pid) FROM forums_post WHERE pfid ='$fid' AND pistopic='0'");
				$total_forum_posts = mysql_result($total_forum_posts, 0);
				$query5 = mysql_query("SELECT p.pdate, p.puid, t.ttitle, t.tid FROM forums_threads AS t, forums_post AS p, users AS u WHERE t.tfid = '$fid' AND p.ptid=t.tid AND u.ID=p.puid ORDER BY p.pid DESC LIMIT 0, 1;"); 
				...
				if ($pdate2 == ''){$pdate1 = '';$ptitle1 = '';$GetLastPost = "<img src='/images/forumimages/lastpost.gif' border='0'  alt='Last Post' />";$Username = '';
					...
					$GetLastPost = "<a href='/forums/$fseo/topic-$ptid/showlastpost' title='Go to the last post'><img src='/images/forumimages/lastpost.gif' border='0' alt='Last Post' /></a>";
				}
		...
		echo "</tr></table>\n";
		echo "<table cellpadding='0' cellspacing='1' width='100%'>\n<tr>\n";
		echo "<td class='tbl2' width='1%' style='white-space: no-wrap;' rowspan='4'><center><img src='/images/forumimages/user.png' border='0' alt='Active Users' /></center></td>\n";
		echo "</tr>\n<tr>\n";
		$FinalDate3 = time()-900;
		$ou24h = mysql_query("SELECT COUNT(ID) FROM users WHERE lastActivity>='$FinalDate3'"); 
		$online_users_24hours = mysql_result($ou24h, 0);
		...
		echo "</td>\n";
		echo "</tr>\n<tr>\n";
		echo "<td class='tbl2'><img src='/images/adminbadge.gif' alt='Admin of Animeftw' style='vertical-align:middle;' /><a href=\"/staff/\">Admin</a> | <a href=\"/staff\">Moderator</a> | <a href=\"/staff/\">Site Staff</a> | <img src='/images/advancedimages/default.gif' alt='Advanced User Title' style='vertical-align:middle;' /><a href='/advanced'>Advanced Member</a> | <a href='/user/'>Basic Member</a></td>\n";
		echo "</tr>\n<tr>\n";
		//Board Statistics
		echo "<td class='tbl2' width='1%' style='white-space: no-wrap;' rowspan='4'><center><img src='/images/forumimages/stats.png' border='0' alt='Board Stats' /></center></td>\n";
		echo "</tr>\n<tr>\n";
	...
	private function SimpleStats(){
		$result = mysql_query("SELECT COUNT(ID) AS UserCount, (SELECT ID FROM users WHERE active='1' ORDER BY id DESC LIMIT 0, 1) AS ChosenUser, (SELECT COUNT(pid) FROM forums_post) AS ForumPosts FROM users WHERE Active='1'");
