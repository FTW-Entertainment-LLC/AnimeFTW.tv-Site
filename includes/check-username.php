...
switch($do) {
	case 'check_username_exists':
		...
		$count = mysql_num_rows(mysql_query("SELECT ID FROM users WHERE Username='".mysql_real_escape_string($username)."' OR `display_name` ='".mysql_real_escape_string($username)."'"));
		if($count > 0) {
